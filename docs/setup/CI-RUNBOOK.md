...
9. **packages 要 rebuild**（lessons #10 同源）：Docker build stage 用 `turbo run build`（連 `@certifine/web-kit` 等），唔可以淨係 build app。
...
12. **Workspace 缺 dependency 宣告 → fresh build TS2307**：`@certifine/api-client` `src/types.ts` import `@certifine/web-kit` 但 package.json 冇宣告 dependency → turbo 唔知要先 build `@certifine/web-kit` 先 type-check api-client。喺 Mac / bind-mount 因為 `dist/` 預先存在冇爆；隔離 fresh build 就 `Cannot find module '@certifine/web-kit' (TS2307)`。**Fix：`packages/api-client/package.json` 加 `"dependencies": { "@certifine/web-kit": "*" }`。每次隔離 build 撞 TS2307「Cannot find module '@authentik/*'」= 去查嗰個 package 有冇漏宣告 workspace dep。**
