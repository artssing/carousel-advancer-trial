...
test.describe('IN-03 已收乾淨嘅 consumer 畫面 lang=en 之後零中文', () => {
  ...
  test('IN-03 /listing/[id]', async ({ page, context }) => {
    ...
    // called 「Chanel Classic 幾乎全新」 is correct in every locale. Since
    // 2026-08-11 those nodes carry `data-user-content` and chineseOnScreen drops
    // them, so this needs no per-page filter of its own.
