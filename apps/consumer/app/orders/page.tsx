...
import { Badge, TierPill, Button, ListingThumb } from '@authentik/ui';
import {
        ...
        return (
          <div key={label} className="flex flex-1 flex-col items-center">
            <div className="flex w-full items-center">
              <div className={`h-0.5 flex-1 ${i === 0 ? 'invisible' : done ? 'bg-brand-500' : 'bg-slate-200'}`} />
              ...
              <div className={`h-0.5 flex-1 ${i === steps.length - 1 ? 'invisible' : done ? 'bg-brand-500' : 'bg-slate-200'}`} />
            </div>
            <span className={`mt-1 text-[9px] font-medium ${done ? 'text-brand-600' : current ? 'text-brand-500' : 'text-slate-400'}`}>
              {label}
            </span>
          </div>
        );
...
export default function OrdersPage() {
  ...
  function renderActions(o: any) {
    ...
    if (activeTab === 'buyer' || (activeTab !== 'auth' && isBuyer)) {
      ...
      // SHIP: buyer confirm-received.
      // Founder ruling 2026-06-19: 買家唔需要影任何相 — confirm 收到就完成交易。
      if (!meetup && o.status === 'SHIPPED_TO_BUYER') {
        btns.push({ label: '確認已收到商品', action: () => api.orders.confirmDelivered(o.id, []), primary: true });
      }
      if (!meetup && o.status === 'DELIVERED')
        btns.push({ label: '確認完成交易', action: () => api.orders.complete(o.id), primary: true });
      ...
      if (o.deliveryMethod === 'MEETUP_3WAY' && o.status === 'AUTH_PASSED') {
        const label = o.escrowHeld
          ? '確認面交完成（放款畀賣家+鑑定師）'
          : '確認面交完成';
        btns.push({ label, action: () => api.orders.completeMeetup(o.id), primary: true });
      }
      // MEETUP_DIRECT: confirm meetup complete
      if (o.deliveryMethod === 'MEETUP_DIRECT' && o.status === 'PAID') {
        btns.push({ label: '確認面交完成', action: () => api.orders.completeMeetup(o.id), primary: true });
      }
    }
    ...
    if (activeTab === 'seller' || (activeTab !== 'auth' && isSeller)) {
      if (!meetup && o.status === 'PAID' && o.authenticatorId)
        btns.push({ label: '確認已寄出至鑑定師', action: () => api.orders.shipToAuthenticator(o.id), primary: true });
      if (!meetup && o.status === 'PAID' && !o.authenticatorId)
        btns.push({ label: '確認已寄出至買家', action: () => api.orders.shipToBuyerDirect(o.id), primary: true });
      if (!meetup && o.status === 'AUTH_PASSED')
        btns.push({ label: '確認已寄出至買家', action: () => api.orders.shipToBuyer(o.id), primary: true });
    }
  ...
  function renderDualAckNav(o: any) {
    ...
    if (isSeller && o.status === 'AUTH_RECEIVED_PENDING_SELLER_ACK') {
      ...
      label = '睇鑑定師接收相 + 確認交付 →';
    } else if (isBuyer && o.status === 'AWAITING_BUYER_PICKUP') {
      label = '去鑑定師店取貨 · 現場確認 →';
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      {/* Header */}
      <div className="mb-4">
        <h1 className="font-display text-2xl font-bold text-slate-900">我的訂單</h1>
      </div>
      ...
      <div className="mb-6 flex gap-1 rounded-xl bg-slate-100 p-1">
        {tabs.map((t) => (
          <button
            ...
            className={`relative flex-1 rounded-lg py-2 text-center text-sm font-medium transition
              ${activeTab === t.id
          ...
          >
            {t.label}
            {!loading && (
              <span className={`ml-1 text-xs ${activeTab === t.id ? 'text-slate-400' : 'text-slate-400'}`}>
                ({t.count})
              </span>
            )}
            {t.actionCount > 0 && (
            ...
            return (
              <div
                ...
                className={`overflow-hidden rounded-2xl border bg-white shadow-sm transition hover:shadow-md ${
                  isAction ? 'border-brand-300 ring-1 ring-brand-100' : 'border-slate-100'
                }`}
              >
                {/* Status colour stripe */}
                <div className={`h-1 w-full ${STATUS_STRIPE(o.status)}`} />

                <div className="p-4">
                  {/* Action needed banner */}
                  {isAction && (
                    <div className="mb-3 flex items-center gap-2 rounded-lg bg-brand-50 px-3 py-1.5 text-xs font-medium text-brand-700">
