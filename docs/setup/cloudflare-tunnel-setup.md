...
| `n8n.certifinehk.com` | `authentik-n8n:5678` | ops ⚠️ Access |

> **n8n（單一 instance，founder 2026-07-21）**：ops 自動化工具，唔分 uat/prod。
> 同 cloudflared 共用 `carousel-advancer-trial_default` 網（docker-compose.ci.yml
> 同一 project），所以 tunnel 直接指 container 名 `authentik-n8n:5678`。
> `WEBHOOK_URL` 已設 `https://n8n.certifinehk.com/`。**必須 Access 鎖**（有
> Telegram token 等 secret）。Jenkins→n8n 內部通知照用 `http://n8n:5678/...`。
