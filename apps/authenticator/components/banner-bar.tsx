...
// Mirror of apps/consumer/components/banner-bar.tsx (portal-parity rule —
// see memory feedback_im_portal_parity). Same behavior, but audience defaults
// to AUTHENTICATORS so ops can push authenticator-specific notices.
...
export function BannerBar() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const items = await api.banners.list('AUTHENTICATORS');
